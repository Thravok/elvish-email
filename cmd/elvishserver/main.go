// Command elvishserver runs the live Elvish site (HTML + /api) backed by CockroachDB/Postgres and Valkey by default.
// Set ELVISH_ALLOW_EMPTY_DB=1 for static-only mode without those backends.
package main
...
func main() {
	...
	if err := maybeBootstrapMFAEncryptionKey(*root, logger); err != nil {
		logger.Error("mfa key bootstrap", "err", err)
		os.Exit(1)
	}

	cfg := db.LoadConfigFromEnv()
	if err := cfg.Validate(); err != nil {
		logger.Error("db config invalid", "err", err)
		os.Exit(1)
	}

	allowEmptyDB := envTruthyMain("ELVISH_ALLOW_EMPTY_DB")
	strictDB := !allowEmptyDB && !*migrateOnly
	if strictDB {
		if cfg.CockroachDSN == "" || cfg.ValkeyAddr == "" {
			logger.Error("elvishserver requires COCKROACH_DSN and VALKEY_ADDR (both must be set and reachable)",
	...
	if cfg.Enabled() {
		...
		logger.Error("internal: strict DB mode but no backends enabled")
		os.Exit(1)
	}
	...
	if *migrateOnly {
		...
		n, err := migrate.PostsFromDisk(ctx, sqlStore, filepath.Join(*root, "content"))
		if err != nil {
	...
	relayPath, relayExplicit := relayKeyPathForRoot(*root)
	...
	if mm != nil && scy != nil && blb != nil {
		...
		dkimDomainsDir := filepath.Join(*root, "data", "dkim", "domains")
