...
import { AdminPanelLayout } from "../shared/layout.jsx";
import { Button } from "../shared/primitives.jsx";
...
/** Keys sent to PUT /api/console/site/home (must match server merge). */
...
const ADMIN_SECTION_DEFS = [
  ...
  },
  {
    ...
    num: "10",
    label: "Support Inbox",
    description: "Read and reply to support@ mail via escrowed keys.",
    key: "SecSupportInbox",
    icon: "mail",
    searchKeywords: ["support", "inbox", "help", "ticket"]
  }
];
...
const ADMIN_COMMANDS = [
  ...
  { id: "goto.support", name: "Goto · Support inbox", scope: "ADMIN", glyph: "▸" },
...
function AdminApp({ embedded, onLeaveEmbedded } = {}) {
  const [staffMe, setStaffMe] = useS_app(null);
  const [staffChecked, setStaffChecked] = useS_app(false);
  ...
  useE_app(() => {
    ...
    const onSession = () => {
      fetch("/api/staff/me", { credentials: "include" })
        .then((r) => r.json())
        .then((j) => setStaffMe(j.staff || null))
        .catch(() => {});
    };
    window.addEventListener("elvish:sessionChanged", onSession);
    return () => {
      cancelled = true;
      window.removeEventListener("elvish:sessionChanged", onSession);
    };
  }, [embedded]);
  ...
  useE_app(() => {
    ...
    (async () => {
      try {
        ...
        if (!r.ok) {
          devLog("admin bootstrap: falling back to /console/bootstrap.json", r.status);
          r = await fetch("/console/bootstrap.json", { cache: "no-store" });
        }
        if (!r.ok || cancelled) {
  ...
  const runCommand = (c) => {
    ...
    if (id === "palette.modals") {
      window.location.href = "/console/modals.html";
      return;
    }
    if (id === "about") {
      setDialog("about");
  ...
  if (!embedded && staffChecked && !staffMe) {
    return (
      <div className="frame" style={{ padding: 48, maxWidth: 480, margin: "80px auto" }}>
        <div className="ftk">// CONSOLE LOGIN</div>
        <p className="dim" style={{ marginTop: 8 }}>Staff credentials required.</p>
        {M && (
          <M.LoginModal
            open
            onClose={() => {}}
            onDone={(j) => {
              setStaffMe((j && j.staff) || { email: "staff" });
      ...
      <div className="frame" data-testid="admin-root" data-admin-embedded={embedded ? "1" : undefined}>
        <header className="admin-topbar">
          <div className="admin-topbar-left">
            ...
            <span className="admin-brand-section">CONSOLE</span>
          </div>
        ...
        <>
          ...
          <M.PublishModal
            ...
            onSaveMongo={
              persistHome
                ? async () => {
                    const r = await fetch("/api/console/site/home", {
                      method: "PUT",
                      credentials: "include",
...
(function mountAdminStandalone() {
  if (document.documentElement.getAttribute("data-console-standalone") !== "1") {
    return;
  }
