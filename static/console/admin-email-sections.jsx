...
function UserDetailModal({ user, onClose, onDisable, onDelete, busy }) {
  ...
  useEf(() => {
    ...
    (async () => {
      try {
        const d = await apiJSON(`/api/console/users/${encodeURIComponent(user.id)}?expand=mail`);
...
function DomainDetailModal({ domain, onClose, onVerify, onDelete, busy, onSharingSaved }) {
  ...
  useEf(() => {
    ...
    (async () => {
      try {
        const d = await apiJSON(`/api/console/domains/${encodeURIComponent(domain.domain)}`);
        if (cancelled) return;
        setDetail(d);
  ...
  useEf(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const data = await apiJSON(`/api/console/users?q=${encodeURIComponent(userQuery.trim())}&limit=12`);
        if (cancelled) return;
  ...
  const saveSharing = async () => {
    ...
    try {
      ...
      await apiJSON(`/api/console/domains/${encodeURIComponent(domain.domain)}/sharing`, {
        method: "PATCH",
...
function SecUsers() {
  ...
  const load = useCb(async () => {
    setLoading(true);
    try {
      const url = `/api/console/users?page=${page}&limit=${ADMIN_USER_PAGE}${query ? `&q=${encodeURIComponent(query)}` : ""}`;
                  ...
                  onClick={async () => {
                    ...
                    try {
                      if (confirmAction.type === "disable") {
                        await apiJSON(`/api/console/users/${confirmAction.user.id}/disable`, { method: "POST" });
                      } else {
                        await apiJSON(`/api/console/users/${confirmAction.user.id}`, { method: "DELETE" });
                      }
...
function SecOutbox() {
  ...
  const load = useCb(async () => {
    setLoading(true);
    try {
      const statsData = await apiJSON("/api/console/outbox/stats");
      setStats(statsData || {});
      const url = `/api/console/outbox?page=${page}&limit=20${status ? `&status=${encodeURIComponent(status)}` : ""}${source ? `&source=${encodeURIComponent(source)}` : ""}`;
          ...
          <tbody>
            ...
            {items.map((it) => (
              <tr key={it.ID}>
                ...
                <td>
                  {it.Status === "failed" && <button className="btn-sm" onClick={async () => { await apiJSON(`/api/console/outbox/${it.ID}/retry`, { method: "POST" }); load(); }}>Retry</button>}
                </td>
              </tr>
            ))}
          </tbody>
...
function SecDomains() {
  ...
  const load = useCb(async () => {
    setLoading(true);
    try {
      const data = await apiJSON(`/api/console/domains?page=${page}&limit=${ADMIN_DOMAIN_PAGE}`);
  ...
  const registerDomain = async (e) => {
    ...
    setAddResult(null);
    try {
      const res = await apiJSON("/api/console/domains", {
        method: "POST",
        body: JSON.stringify({
          domain: form.domain,
  ...
  const verifyDomain = async (domainName) => {
    ...
    setMsg("");
    try {
      const res = await apiJSON(`/api/console/domains/${encodeURIComponent(domainName)}/verify`, { method: "POST" });
      const checks = [
  ...
  const executeDeleteDomain = async () => {
    ...
    setMsg("");
    try {
      await apiJSON(`/api/console/domains/${encodeURIComponent(domainName)}`, { method: "DELETE" });
...
function SecMailTest() {
  ...
  const runHealth = async () => {
    ...
    try {
      setHealth(await apiJSON("/api/console/test/health"));
    } catch (e) {
  ...
  const runReadiness = async () => {
    ...
    try {
      setReadiness(await apiJSON("/api/console/test/readiness"));
    } catch (e) {
  ...
  const runAuthPosture = async () => {
    ...
    try {
      setAuthPosture(await apiJSON("/api/console/test/auth-posture"));
    } catch (e) {
  ...
  const runPrivacyPosture = async () => {
    ...
    try {
      setPrivacyPosture(await apiJSON("/api/console/test/privacy-posture"));
    } catch (e) {
  ...
  const loadKeyMaterial = async () => {
    ...
    try {
      const data = await apiJSON("/api/console/test/key-material");
      setKeyMaterial(data);
      setForm((f) => ({
        ...f,
  ...
  const loadMailConfig = async () => {
    try {
      const data = await apiJSON("/api/console/system-mail");
      setMailConfig(data);
  ...
  const runEcho = async () => {
    ...
    try {
      setEchoResult(await apiJSON("/api/console/test/echo", {
        method: "POST",
        body: JSON.stringify({
  ...
  const runKeyserver = async () => {
    ...
    try {
      setKeyserverResult(await apiJSON("/api/console/test/keyserver-probe", {
        method: "POST",
  ...
  const runWrap = async () => {
    ...
    try {
      setWrapResult(await apiJSON("/api/console/test/wrap-roundtrip", {
        method: "POST",
  ...
  const searchTestUsers = async () => {
    ...
    try {
      const data = await apiJSON(`/api/console/users?page=1&limit=10&q=${encodeURIComponent(testSearchQuery)}`);
  ...
  const ensurePlaintextUploads = useCb(async () => {
    ...
    for (let i = 0; i < next.length; i += 1) {
      ...
      try {
        const uploaded = await apiForm("/api/console/test/uploads", fd);
        next[i] = {
          ...next[i],
          uploading: false,
  ...
  const runTestPreview = useCb(async () => {
    ...
    try {
      ...
      setTestPreview(await apiJSON("/api/console/test/preview", {
        method: "POST",
  ...
  const runTestSend = useCb(async () => {
    ...
    try {
      ...
      const res = await apiJSON("/api/console/test/send", {
        method: "POST",
        ...
        <div className="test-card">
          ...
          <div className="mail-actions">
            <button className="btn-sm" onClick={async () => {
              ...
              closeKeyAlert();
              try {
                const res = await apiJSON("/api/console/test/key-material/dkim/config", {
                  method: "POST",
                  body: JSON.stringify({
            ...
            <button className="btn-sm" onClick={async () => {
              ...
              closeKeyAlert();
              try {
                const res = await apiJSON("/api/console/test/key-material/dkim/upload", {
                  method: "POST",
                ...
                onClick={async () => {
                  ...
                  try {
                    if (confirmKeyAction === "relay") {
                      const res = await apiJSON("/api/console/test/key-material/relay/generate", { method: "POST", body: "{}" });
                      setKeyMsg(`Relay key active: ${String(res.fingerprint || "").slice(-16)}.`);
                    } else {
                      const res = await apiJSON("/api/console/test/key-material/dkim/generate", { method: "POST", body: "{}" });
...
function SecSystemMail() {
  ...
  const load = useCb(async () => {
    try {
      const cfg = await apiJSON("/api/console/system-mail");
      setConfig(cfg);
      setRuns((await apiJSON("/api/console/system-mail/runs")).items || []);
  ...
  const searchUsers = async () => {
    setSearching(true);
    try {
      const data = await apiJSON(`/api/console/users?page=1&limit=10&q=${encodeURIComponent(searchQuery)}`);
      ...
      <A.Card title="COMPOSE SYSTEM MAIL" right={config ? `${config.active_user_count || 0} active users` : "loading…" }>
        ...
        <div className="mail-actions">
          <button className="btn-sm" onClick={async () => {
            ...
            try {
              setPreview(await apiJSON("/api/console/system-mail/preview", {
                method: "POST",
                body: JSON.stringify(payload)
              }));
            } catch (e) {
          ...
          <button className="btn-sm primary" disabled={!preview || sending || (needsConfirm && !confirmBulk)} onClick={async () => {
            ...
            try {
              const res = await apiJSON("/api/console/system-mail/send", {
                method: "POST",
                body: JSON.stringify(payload)
              });
