...
function ToolEditor({ tool, onChange, onClose, onDelete }) {
  ...
  const runProbeTest = async () => {
    ...
    setProbeBusy(true);
    try {
      const r = await fetch("/api/console/uptime/test-probe", {
        method: "POST",
        credentials: "include",
...
function SecSigningPGP() {
  ...
  return (<>
    ...
    <div className="adm-explain">
      Keys are stored in MongoDB and published at <code className="mono">/pgp/keys.json</code> and <code className="mono">/pgp/key/&lt;fp16&gt;.asc</code>.
      Upload requires an <a href="/login">admin</a> session. Import disk posts with <code className="mono">go run ./cmd/elvishserver -root . -migrate</code> (Mongo) or <code className="mono">POST /api/migrate/posts</code> as admin.
    </div>
