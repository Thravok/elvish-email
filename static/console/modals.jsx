  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/staff/login", {
        ...
        credentials: "include",
        body: JSON.stringify({ email: username, password: p })
      });
      ...
      setPhase("ok");
      const disp = (j.staff && j.staff.email) || username;
    ...
    <Modal open={open} onClose={onClose} title="PUBLISH · REVIEW" kind="PUBLISH" status={`Δ ${(diff||[]).length} CHANGES`} size="wide">
      <div className="mod-head-block">
        ...
        <div className="txt">
          ...
          <p>
            {persistHome
              ? <>Writes the reviewed bundle to <code>site_config.home_json</code> via <code>PUT /api/console/site/home</code>. The public site picks it up after the content cache TTL.</>
      ...
      <div className="mod-foot" style={{ margin: "20px -24px -22px", borderTop: "1px solid var(--fg)" }}>
        <div className="l">{persistHome ? <code>/api/console/site/home</code> : <span className="dim">no Mongo site store</span>}</div>
        <div className="r">
