...
Continuous integration runs on **GitLab CI** via [.gitlab-ci.yml](.gitlab-ci.yml) (`gofmt`, `go vet`, `golangci-lint`, [`scripts/lint-invariants.sh`](scripts/lint-invariants.sh), OpenAPI check, `lint-static-js` bundle freshness, **`lint-docs`** MkDocs strict build, `go test` / `go test -race`, Flutter client tests, release build of `elvishapi`/`elvishmta`/`elvishworker`, and optional stages defined there). **GitHub Actions** runs CodeQL, iOS, Android, and documentation workflows — see [docs/repo-layout.md](docs/repo-layout.md#ci-map). Issue and merge request description templates live under [.gitlab/issue_templates/](.gitlab/issue_templates/) and [.gitlab/merge_request_templates/](.gitlab/merge_request_templates/).
...
| `make docs-check` | Strict MkDocs build (CI gate; fails on broken internal links). |
