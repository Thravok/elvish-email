...
import (
	...
	"elvish/libs/go/feeds"
	"elvish/libs/go/glyphs"
...
func (s *Server) signingPubDeployed() bool {
	pub := filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog", "signing.pub")
...
func (s *Server) handleSigningPub(w http.ResponseWriter, r *http.Request) {
	...
	path := filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog", "signing.pub")
...
func (s *Server) serveAdminOrStatic(w http.ResponseWriter, r *http.Request) {
	...
	if rel != "" && isAdminStaticAsset(rel) {
		full := filepath.Join(paths.RepoRoot(s.root).AdminApp(), filepath.FromSlash(rel))
		fi, err := os.Stat(full)
		if err != nil || fi.IsDir() {
			http.NotFound(w, r)
			return
	...
	if s.adminOrigin != "" {
		http.Redirect(w, r, s.adminOrigin+"/", http.StatusFound)
		return
	}
