...
func (s *Server) apiBootstrap(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	raw, err := os.ReadFile(filepath.Join(paths.RepoRoot(s.root).APIContent(), "home.json"))
	if err != nil {
...
func (s *Server) apiMigratePosts(w http.ResponseWriter, r *http.Request) {
	...
	n, err := migrate.PostsFromDisk(r.Context(), s.store, filepath.Join(paths.RepoRoot(s.root).APIContent()))
	if err != nil {
