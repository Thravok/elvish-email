...
func (s *Server) loadMetrics() (map[string]blog.EntryMetrics, error) {
	p := filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog", "metrics.json")
...
func (s *Server) loadPostsUncached(ctx context.Context) ([]blog.Post, error) {
	...
	pubPath := filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog", "signing.pub")
	if fi, err := os.Stat(pubPath); err == nil && !fi.IsDir() {
		signingOpts = &blog.SigningLoadOpts{VerifyPubPath: pubPath}
	}
	if s.store == nil {
		return blog.LoadPosts(filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog"), md, metrics, signingOpts)
	}
	...
	if n == 0 {
		return blog.LoadPosts(filepath.Join(paths.RepoRoot(s.root).APIContent(), "blog"), md, metrics, nil)
	}
